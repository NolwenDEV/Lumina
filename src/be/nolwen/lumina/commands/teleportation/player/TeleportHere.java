...
@CommandRegistrar(NAME = "teleporthere")
public class TeleportHere implements CommandExecutor {
	
	@Override
	public boolean onCommand(CommandSender sender, Command command, String label, String[] arguments) {
		if(!command.getName().equalsIgnoreCase("teleporthere")) return true;
		...
		if(arguments.length < 1) return Utils.error(sender, Main.getInstance().getLanguageManager().getTeleportHereUsageError());
		...
		if(!Main.getInstance().getDataManager().getTeleportRequests().containsKey(target)
			...
			target.sendMessage(Utils.format(Main.getInstance().getLanguageManager().getTeleportationRequestReceivedMessage(),
					Map.of("PLAYER", player.getName(), "COMMAND", "tpto")
			));
