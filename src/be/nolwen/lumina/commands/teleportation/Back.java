...
public class Back implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] arguments) {
		...
		if(!Main.getInstance().getConfigManager().isBackModule()) return MessageBuilder.error(sender, Main.getInstance().getLanguageManager().getFeatureDisabledError());
		...
		if(!Main.getInstance().getDataManager().getDeadPlayers().containsKey(player))
			return MessageBuilder.error(sender, Main.getInstance().getLanguageManager().getBackNotDeadError());
